...
from model_bst import build_model  # IMPORT BST
...
if not os.path.exists(config_path): config_path = "config/bst_config.yaml"
...
print("Construction modèle BST...")
model = build_model(None, model_cfg)

checkpoint_path = "../checkpoints/BST_best.pth"
if not os.path.exists(checkpoint_path): checkpoint_path = "checkpoints/BST_best.pth"
...
sub.to_csv("prediction_bst.csv", index=False)

with zipfile.ZipFile("submission_bst.zip", 'w', zipfile.ZIP_DEFLATED) as zf:
    zf.write("prediction_bst.csv")

print("✅ Fichier 'submission_bst.zip' généré avec succès !")