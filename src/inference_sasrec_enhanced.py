...
from model_sasrec_enhanced import build_model
...
# Run
config_path = "../config/sasrec_enhanced_config.yaml"
if not os.path.exists(config_path): config_path = "config/sasrec_enhanced_config.yaml"
...
print("Construction SASRec++...")
model = build_model(None, model_cfg)

checkpoint_path = "../checkpoints/SASRec_Enh_best.pth"
if not os.path.exists(checkpoint_path): checkpoint_path = "checkpoints/SASRec_Enh_best.pth"
...
sub.to_csv("prediction_sasrec_enh.csv", index=False)
with zipfile.ZipFile("submission_sasrec_enh.zip", 'w', zipfile.ZIP_DEFLATED) as zf:
    zf.write("prediction_sasrec_enh.csv")
print("✅ Terminé !")